parser grammar NodeScriptParser;

options {
  output = AST; // Produce a tree of node
  tokenVocab = NodeScriptLexer; // Use tokens defined in our lexer.
...
literal returns [Node node]:
  ...
  | NIL               { $node = new LiteralNode(NodeScriptRuntime.getNil()); }
  | TRUE              { $node = new LiteralNode(NodeScriptRuntime.getTrue()); }
  | FALSE             { $node = new LiteralNode(NodeScriptRuntime.getFalse()); }
